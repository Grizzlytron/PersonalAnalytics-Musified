...
export class SchedulingService {
  ...
  public updateRetrospectionJobs(workSchedule: WorkHoursDto): void {
    ...
    const daysOfWeek: (keyof WorkHoursDto)[] = [
      'sunday',
      'monday',
      'tuesday',
      'wednesday',
      ...
      if (daySchedule.isWorking) {
        ...
        const job = schedule.scheduleJob(
          ...
          async (fireDate: Date): Promise<void> => {
            LOG.info(
              `Retrospection Job was supposed to fire at ${fireDate}, fired at ${new Date()}`
            );
    ...
    LOG.info(
      `Updated weekly retrospection jobs: ${this.retrospectionJobs.map((j) => j.nextInvocation()).join(', ')}`
    );
  }
  ...
  private async handleRetrospectionJob(): Promise<void> {
    ...
    const settings: Settings | null = await Settings.findOne({
      where: { onlyOneEntityShouldExist: 1 }
    });
    if (!settings) {
